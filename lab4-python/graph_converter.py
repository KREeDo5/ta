...
def convert_to_dfa(nfa):
    ...
    start_state = nfa.graph['start']  # Получаю начальное состояние NFA
    print("начальное состояние:", start_state)

    epsilon_closure = get_epsilon_closure(nfa, {start_state}) # Нахожу ε-замыкание начального состояния
    print("ε-замыкание начального состояния:", epsilon_closure)

    state_queue = [frozenset(epsilon_closure)] #очередь для обработки новых состояний DFA
    ...
    while state_queue:
        ...
        # Проверяю финальность текущего состояния
        ...
        # Получаю все возможные входные символы (кроме ε)
        ...
        for signal in signals:
            # Собираю переходы, достижимые из текущего состояния по входному сигналу
            next_states = set()
            ...
            # Рассчитываю ε-замыкание для полученных состояний
            ...
            #Если новое состояние еще не добавлялось
